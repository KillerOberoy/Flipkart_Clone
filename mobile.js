...
function clearFilter() {
  ...
  displayMobileToDisplay(mobile.products);
}

// Function to handle adding items to the cart
let cartProductArray=JSON.parse(localStorage.getItem("cartData")) || [];
function handleAddToCart(product) {
  return function () {
    // alert(product.title + " added to cart!");
    console.log(product);
    cartProductArray.push(product);
    document.getElementById("cartData").innerHTML=`${cartProductArray.length}`;
    document.getElementById("cartData").style.color="blue";
    localStorage.setItem("cartData",JSON.stringify(cartProductArray));
    
  };
}
function cartShownAfterReload(){
document.getElementById("cartData").innerHTML=`${cartProductArray.length}`;
document.getElementById("cartData").style.color="blue";
}
cartShownAfterReload();

// --------------------------- Right Side Main Section----------------------------
function displayMobileToDisplay(products) {
  products.map((e) => {
    // ------------------------Left Box----------------------------
    ...
    thumbnail.className = "mobileThumbnail";
    ...
    addToCart.addEventListener("click", handleAddToCart(e)); // Attach click event

    middleBox.append(title, rating, warranty, description, addToCart);
    // ---------------------------Right Box---------------------------------
    ...
    price.className = "mobilePrice";

    let discountPercentage = document.createElement("span");
    discountPercentage = e.discountPercentage;

    let ship = document.createElement("span");
    ship.innerHTML = e.shippingInformation;

    let avail = document.createElement("span");
    avail.innerHTML = e.availabilityStatus;

    rightBox.append(price, discountPercentage, ship, avail);

    let hr = document.createElement("hr");
    document.getElementById("collection").append(leftBox, middleBox, rightBox);
  });
}
displayMobileToDisplay(mobile.products);
...
// -----------------------Filter By Ratings------------------------------------------
function rating4() {
  let filteredMobile = mobile.products.filter((e) => {
    return e.rating >= 4;
  });
  document.getElementById("collection").innerHTML = "";
  displayMobileToDisplay(filteredMobile);
}
